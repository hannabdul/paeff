...
from retrieval_model import PAEFF

face = 'facenet'

def read_data(FLAGS):
    
    test_file_face = '/home/prashantpatil/Documents/Mustansar/sep_24/jan2025/face_voice/voxceleb1_feats/face_veriflist_test_%s_unseenunheard.csv'%(FLAGS.test)
    test_file_voice = '/home/prashantpatil/Documents/Mustansar/sep_24/jan2025/face_voice/voxceleb1_feats/voice_veriflist_test_%s_unseenunheard.csv'%(FLAGS.test)
    ...
    face_test = np.asarray(face_test).astype(np.float32)
    face_test = face_test[:,:4096]

    voice_test = np.asarray(voice_test).astype(np.float32)
...
def test(face_test, voice_test):
    ...
    model = PAEFF(FLAGS, face_test.shape[1], voice_test.shape[1])
    ...
    with torch.no_grad():
        _, face, voice, _, _ = model(face_test, voice_test)
    ...
    #    eer = brentq(lambda x: 1. - x - interpolate.interp1d(fpr, tpr)(x), 0., 1.)
        eer_unheard = brentq(lambda x : 1. - x - interpolate.interp1d(fpr, tpr)(x), 0., 1.)
        print('Equal Error Rate (EER): %0.3f --- %0.3f\n\n' % (eer, eer_unheard) )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', type=str, default='../face_voice/model_alpha_1.00/checkpoint_0017.pth.tar', help='Checkpoints directory.')
